...
impl RiskLimits {
    pub fn from_env() -> Result<Self, RiskError> {
        ...
        if let Ok(v) = std::env::var("RISK_MAX_POSITIONS") {
            limits.max_positions = v
                .parse()
        ...
        {
            limits.max_single_position_qty = v
                .parse()
                .map_err(|_| RiskError::Invalid("RISK_POSITION_SAFETY_CEILING invalid".into()))?;
        }
        if let Ok(v) = std::env::var("RISK_MAX_SPREAD_BPS") {
            limits.max_spread_bps = v
                .parse()
                .map_err(|_| RiskError::Invalid("RISK_MAX_SPREAD_BPS invalid".into()))?;
        }
        if let Ok(v) = std::env::var("RISK_MAX_SYMBOL_EXPOSURE") {
            limits.max_symbol_exposure = v
                .parse()
...
impl RiskGovernor {
    ...
    pub fn authorize(
        ...
        if order.qty == 0 {
            return Err(RiskError::Invalid(
                "order quantity must be greater than zero".into(),
            ));
        }
        ...
        if !self.limits.max_order_notional.is_finite()
            || self.limits.max_order_notional <= 0.0
            || !self.limits.max_total_notional.is_finite()
            || self.limits.max_total_notional <= 0.0
            || !self.limits.max_symbol_exposure.is_finite()
            || self.limits.max_symbol_exposure <= 0.0
        {
            return Err(RiskError::Invalid("invalid risk limits".into()));
        }
        ...
        // Genuine safety control: insufficient buying power / cash
        if !order.reduce_only && portfolio.cash > 0.0 && n > portfolio.cash {
            return Err(RiskError::Limit("INSUFFICIENT_BUYING_POWER".into()));
        }

        // Validate resulting order against strategy's allocated risk percentage and budget if registered
        if let Some(strat_risk) = self.strategy_risks.get(&order.strategy_id) {
            ...
            if !order.reduce_only && n > strat_risk.capital_allocated * 1.05 {
                return Err(RiskError::Limit(format!(
                    "Order notional {:.2} exceeds strategy capital allocation {:.2}",
                    n, strat_risk.capital_allocated
                )));
            }
        }
