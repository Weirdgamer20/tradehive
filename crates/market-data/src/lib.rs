...
fn parse_occ_expiry(symbol: &str) -> Option<DateTime<Utc>> {
    ...
    let date = chrono::NaiveDate::from_ymd_opt(p.year as i32, p.month, p.day)?;
    ...
    async fn option_chain(
        ...
        loop {
            ...
            for (symbol, x) in page.snapshots {
                ...
                let parsed = th_domain::occ::parse(&symbol);
...
pub fn synthetic_option_chain(underlying: &str, spot: f64, now: DateTime<Utc>) -> OptionChain {
    let expiry = now + Duration::minutes(240);
    let mut quotes = Vec::new();
    for i in -10..=10 {
...
impl MarketDataProvider for SyntheticProvider {
    async fn bars(
        ...
        Err(MarketDataError::Unavailable(
            "synthetic provider has no historical API".into(),
        ))
    }
    async fn option_chain(
        &self,
